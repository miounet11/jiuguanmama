...
exports.importValidators = void 0;
const zod_1 = require("zod");
// Zod schemas for validation
const sillyTavernEntrySchema = zod_1.z.object({
    uid: zod_1.z.union([zod_1.z.number(), zod_1.z.string()]).optional(),
    key: zod_1.z.array(zod_1.z.string()).min(1, '至少需要一个关键词'),
    keysecondary: zod_1.z.array(zod_1.z.string()).optional(),
    comment: zod_1.z.string().optional(),
    content: zod_1.z.string().min(1, '内容不能为空'),
    constant: zod_1.z.boolean().optional(),
    selective: zod_1.z.boolean().optional(),
    order: zod_1.z.number().optional(),
    position: zod_1.z.enum(['before_char', 'after_char']).optional(),
    disable: zod_1.z.boolean().optional(),
    addMemo: zod_1.z.boolean().optional(),
    excludeRecursion: zod_1.z.boolean().optional(),
    delayUntilRecursion: zod_1.z.boolean().optional(),
    displayIndex: zod_1.z.number().optional(),
    probability: zod_1.z.number().min(0).max(100).optional(),
    group: zod_1.z.string().optional(),
    groupOverride: zod_1.z.boolean().optional(),
    groupWeight: zod_1.z.number().optional(),
    scanDepth: zod_1.z.number().min(0).max(1000).optional(),
    caseSensitive: zod_1.z.boolean().optional(),
    matchWholeWords: zod_1.z.boolean().optional(),
    useGroupScoring: zod_1.z.boolean().optional(),
    automation_id: zod_1.z.string().optional(),
    role: zod_1.z.number().optional(),
    sticky: zod_1.z.number().optional(),
    cooldown: zod_1.z.number().optional(),
    delay: zod_1.z.number().optional()
});
const sillyTavernWorldInfoSchema = zod_1.z.object({
    entries: zod_1.z.array(sillyTavernEntrySchema),
    name: zod_1.z.string().optional(),
    description: zod_1.z.string().optional(),
    version: zod_1.z.string().optional()
});
const worldInfoEntrySchema = zod_1.z.object({
    title: zod_1.z.string().min(1, '标题不能为空').max(200, '标题不能超过200字符'),
    content: zod_1.z.string().min(1, '内容不能为空').max(10000, '内容不能超过10000字符'),
    keywords: zod_1.z.array(zod_1.z.string()).min(1, '至少需要一个关键词'),
    priority: zod_1.z.number().int().min(0).max(999).optional(),
    insertDepth: zod_1.z.number().int().min(0).max(10).optional(),
    probability: zod_1.z.number().min(0).max(1).optional(),
    matchType: zod_1.z.enum(['exact', 'contains', 'regex', 'starts_with', 'ends_with', 'wildcard']).optional(),
    caseSensitive: zod_1.z.boolean().optional(),
    isActive: zod_1.z.boolean().optional(),
    triggerOnce: zod_1.z.boolean().optional(),
    excludeRecursion: zod_1.z.boolean().optional(),
    category: zod_1.z.string().optional(),
    group: zod_1.z.string().optional(),
    position: zod_1.z.enum(['before', 'after', 'replace']).optional()
});
const scenarioSchema = zod_1.z.object({
    name: zod_1.z.string().min(1, '剧本名称不能为空').max(100, '剧本名称不能超过100字符'),
    description: zod_1.z.string().max(500, '描述不能超过500字符').optional(),
    content: zod_1.z.string().max(50000, '内容不能超过50000字符').optional(),
    isPublic: zod_1.z.boolean().optional(),
    tags: zod_1.z.array(zod_1.z.string()).optional(),
    category: zod_1.z.string().max(50, '分类不能超过50字符').optional(),
    language: zod_1.z.string().max(10, '语言代码不能超过10字符').optional(),
    worldInfos: zod_1.z.array(worldInfoEntrySchema).optional()
});
const tavernAIDataSchema = zod_1.z.union([
    scenarioSchema,
    zod_1.z.array(scenarioSchema)
]);
class ImportValidators {
     ...
     * 验证导入数据
     */
    async validateImportData(data, format) {
     ...
     * 检测导入冲突
     */
    async detectConflicts(existingScenarios, importScenarios) {
        const duplicateNames = [];
        const similarContent = [];
        const keywordOverlaps = [];
        // 检测重名
        for (const importScenario of importScenarios) {
            const duplicate = existingScenarios.find(existing => existing.name.toLowerCase() === importScenario.name.toLowerCase());
            if (duplicate) {
                duplicateNames.push({
        ...
        for (const importScenario of importScenarios) {
            for (const existing of existingScenarios) {
                const similarity = this.calculateContentSimilarity(importScenario.description || importScenario.content || '', existing.description || existing.content || '');
        ...
        // 检测关键词重叠
        for (const importScenario of importScenarios) {
            if (!importScenario.worldInfos)
                continue;
            for (const importEntry of importScenario.worldInfos) {
                for (const existing of existingScenarios) {
                    if (!existing.worldInfos)
                        continue;
                    for (const existingEntry of existing.worldInfos) {
                        const overlap = this.calculateKeywordOverlap(importEntry.keywords || [], existingEntry.keywords || []);
     ...
     * 验证SillyTavern格式
     */
    async validateSillyTavernFormat(data, errors, warnings) {
        try {
            // 检查是否为数组或单个对象
     ...
     * 验证SillyTavern条目
     */
    validateSillyTavernEntry(entry, path, errors, warnings) {
        // 检查必要字段
     ...
     * 验证JSON格式
     */
    async validateJSONFormat(data, errors, warnings) {
        try {
            // 如果是字符串，尝试解析JSON
            let parsedData = data;
     ...
     * 验证YAML格式
     */
    async validateYAMLFormat(data, errors, warnings) {
        try {
            // YAML数据应该已经被解析为对象
     ...
     * 验证增强格式
     */
    async validateEnhancedFormat(data, errors, warnings) {
        try {
     ...
     * 创建验证结果
     */
    createValidationResult(isValid, errors, warnings, format, totalItems, startTime) {
        return {
            isValid,
            errors,
            warnings,
...
exports.importValidators = new ImportValidators();
//# sourceMappingURL=importValidators.js.map