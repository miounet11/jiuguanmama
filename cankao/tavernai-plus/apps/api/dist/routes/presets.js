...
router.get('/', auth_1.optionalAuth, async (req, res, next) => {
    try {
        ...
        // 获取用户自定义预设（如果存在数据库表）
        const customPresets = []; // TODO: 从数据库获取
        // 合并内置预设和用户预设
        ...
        if (aiModel) {
            allPresets = allPresets.filter(p => p.metadata.aiModel === 'any' || p.metadata.aiModel === aiModel);
        }
        res.json({
            success: true,
            presets: allPresets,
...
router.post('/', auth_1.authenticate, async (req, res, next) => {
    try {
        const { name, description, isPublic = false, parameters, metadata } = req.body;
        // 验证参数
