...
const newapi_image_generator_1 = __importDefault(require("../services/newapi-image-generator"));
 ...
 * 为指定角色生成头像
 */
router.post('/characters/:id/generate-avatar', auth_1.authenticate, validateCharacterId, validation_1.validateRequest, async (req, res) => {
    try {
        const { id } = req.params;
        ...
        try {
            ...
            // 更新角色数据
            const updatedCharacter = await prisma_1.prisma.character.update({
                where: { id },
                data: {
                    avatar: avatarUrl,
 ...
 * 为指定角色生成对话背景图
 */
router.post('/characters/:id/generate-background', auth_1.authenticate, validateCharacterId, validation_1.validateRequest, async (req, res) => {
    try {
        const { id } = req.params;
        const userId = req.user?.id;
        // 检查角色是否存在且用户有权限
 ...
 * 重新生成角色的所有图片
 */
router.post('/characters/:id/regenerate-images', auth_1.authenticate, validateCharacterId, validation_1.validateRequest, async (req, res) => {
    try {
        const { id } = req.params;
        const userId = req.user?.id;
        // 检查角色是否存在且用户有权限
 ...
 * 批量生成角色图片（管理员功能）
 */
router.post('/admin/characters/batch-generate', auth_1.authenticate, validateBatchGenerate, validation_1.validateRequest, async (req, res) => {
    try {
        const userId = req.user?.id;
            ...
            await Promise.all(batch.map(async (character) => {
                ...
                try {
                    ...
                    if (type === 'avatar' || type === 'both') {
                        ...
                        promises.push(imageGenerator.generateAvatar(character).then(async (url) => {
                            await prisma_1.prisma.character.update({
                                where: { id: character.id },
                    ...
                    if (type === 'background' || type === 'both') {
                        ...
                        promises.push(imageGenerator.generateBackground(character).then(async (url) => {
                            await prisma_1.prisma.character.update({
                                where: { id: character.id },
 ...
 * 获取角色图片生成状态
 */
router.get('/characters/:id/generation-status', auth_1.authenticate, validateCharacterId, validation_1.validateRequest, async (req, res) => {
    try {
        const { id } = req.params;
