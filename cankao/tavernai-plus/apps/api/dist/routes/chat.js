...
router.get('/:id', auth_1.authenticate, async (req, res, next) => {
    try {
        ...
        if (!session) {
            // 获取角色关联的剧本
            const activeScenarios = await characterScenarioService.resolveActiveScenarios(characterId, undefined, // 新会话还没有chatId
            {
            ...
            // 提取活跃的世界信息
            const worldInfoEntries = await characterScenarioService.activateWorldInfoEntries(activeScenarios, [], // 新会话没有消息历史
            20);
...
router.post('/:characterId/messages', auth_1.authenticate, async (req, res, next) => {
    try {
        ...
        if (isUUID) {
            // 传入的是会话ID，先查找会话再获取角色
            session = await prisma_1.prisma.chatSession.findUnique({
                where: {
                    id: characterId,
                    userId: req.user.id
                },
            ...
            // 传入的是角色ID，验证角色是否存在
            character = await prisma_1.prisma.character.findUnique({
                where: { id: characterId },
                include: { creator: true }
            });
            if (!character) {
        ...
        if (!isUUID) {
            ...
            if (!session) {
                // 获取角色关联的剧本
                const activeScenarios = await characterScenarioService.resolveActiveScenarios(character.id, undefined, // 新会话还没有chatId
                {
                    type: 'character_first',
                    globalScenariosEnabled: true,
                    maxActiveScenarios: 10
...
// 获取对话的活跃世界信息
router.get('/:sessionId/world-info', auth_1.authenticate, async (req, res, next) => {
    try {
        const { sessionId } = req.params;
        // 验证会话属于当前用户
        const session = await prisma_1.prisma.chatSession.findFirst({
        ...
        // 获取当前活跃的剧本和世界信息
        const activeScenarios = await characterScenarioService.getCachedScenarios(session.characterId, sessionId, {
            type: 'character_first',
            globalScenariosEnabled: true,
            maxActiveScenarios: 10
        });
        const worldInfoEntries = await characterScenarioService.activateWorldInfoEntries(activeScenarios, messages, 20);
        res.json({
            success: true,
            worldInfo: {
...
// 动态启用/禁用剧本
router.post('/:sessionId/world-info/toggle', auth_1.authenticate, async (req, res, next) => {
    try {
        const { sessionId } = req.params;
