...
// 获取角色推荐
router.get('/characters', auth_1.authenticate, async (req, res) => {
    try {
        const userId = req.user.id;
        const { limit = 10, algorithm = 'hybrid', minScore = 0.1, category, includeExplored = false } = req.query;
        const options = {
            limit: Number(limit),
            algorithm: algorithm,
...
// 获取内容推荐
router.get('/posts', auth_1.authenticate, async (req, res) => {
    try {
        const userId = req.user.id;
        const { limit = 10, algorithm = 'hybrid', minScore = 0.1, contentType, includeExplored = false } = req.query;
        const options = {
            limit: Number(limit),
            algorithm: algorithm,
            minScore: Number(minScore),
...
// 记录用户行为
router.post('/behavior', auth_1.authenticate, async (req, res) => {
    try {
        const userId = req.user.id;
        ...
        // 记录用户行为
        const behavior = await prisma_1.prisma.userBehavior.create({
            data: {
                userId,
                action,
                targetType,
...
router.put('/preferences', auth_1.authenticate, async (req, res) => {
    try {
        ...
        // 创建新偏好
        const newPreferences = await Promise.all(preferences.map((pref) => prisma_1.prisma.userPreference.create({
            data: {
                userId,
...
router.get('/stats', auth_1.authenticate, async (req, res) => {
    try {
        const userId = req.user.id;
        // 获取用户行为统计
        const behaviorStats = await prisma_1.prisma.userBehavior.groupBy({
            by: ['action'],
            where: { userId },
            _count: { action: true },
        ...
        // 获取推荐点击率
        const recommendations = await prisma_1.prisma.recommendationFeedback.findMany({
            where: { userId },
            select: {
                clicked: true,
                useful: true,
        ...
        // 获取模型性能
        const modelPerformance = await prisma_1.prisma.modelPerformance.findMany({
            where: { userId },
...
router.post('/feedback', auth_1.authenticate, async (req, res) => {
    try {
        ...
        const feedback = await prisma_1.prisma.recommendationFeedback.create({
            data: {
                userId,
                recommendationId,
                clicked,
...
router.get('/trending', async (req, res) => {
    try {
        ...
        if (type === 'characters') {
            // 获取热门角色（基于最近的互动数）
            trending = await prisma_1.prisma.character.findMany({
                include: {
                    _count: {
                        select: {
                            conversations: {
            ...
            // 获取热门内容（基于最近的点赞和评论）
            trending = await prisma_1.prisma.post.findMany({
                where: {
                    createdAt: { gte: since }
                },
                include: {
...
// 重新训练推荐模型（管理员功能）
router.post('/retrain', auth_1.authenticate, async (req, res) => {
    try {
        // 检查管理员权限
