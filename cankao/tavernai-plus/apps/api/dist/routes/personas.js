...
router.get('/', auth_1.authenticate, async (req, res, next) => {
    try {
        ...
        let allPersonas = [...customPersonas];
        // 添加内置人格模板
        if (includeBuiltin) {
...
router.post('/', auth_1.authenticate, async (req, res, next) => {
    try {
        const { name, description, personality, background, speakingStyle, interests = [], traits = [], systemPrompt, isDefault = false, metadata = {} } = req.body;
        // 验证必要字段
...
// 设置默认人格
router.post('/:id/set-default', auth_1.authenticate, async (req, res, next) => {
    try {
        const { id } = req.params;
        // TODO: 验证人格存在
        // TODO: 将当前默认人格设为非默认
...
// 获取当前激活的人格
router.get('/active/current', auth_1.authenticate, async (req, res, next) => {
    try {
...
// 激活人格（用于当前会话）
router.post('/:id/activate', auth_1.authenticate, async (req, res, next) => {
    try {
        const { id } = req.params;
        // TODO: 验证人格存在
        // TODO: 在会话中设置激活的人格ID
