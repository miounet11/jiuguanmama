...
// 验证schema
const updateUserModeSchema = zod_1.z.object({
    currentMode: zod_1.z.enum(['simplified', 'expert']),
    reason: zod_1.z.string().optional()
});
const recordFeatureUsageSchema = zod_1.z.object({
    featureId: zod_1.z.string().min(1),
    isExpertFeature: zod_1.z.boolean().optional().default(false)
});
const analyzeUpgradeSchema = zod_1.z.object({
...
router.get('/', auth_1.authenticate, async (req, res) => {
    try {
        ...
        const featuresUsed = recentUsage.map((usage) => usage.featureId);
        const expertFeaturesUsed = recentUsage
            .filter((usage) => usage.isExpertFeature)
        ...
        const response = {
            ...
            unlockedFeatures: unlockedFeatures.map((unlock) => ({
 ...
 * 更新用户模式
 */
router.put('/', auth_1.authenticate, (0, validate_1.validate)(updateUserModeSchema), async (req, res) => {
    try {
        const userId = req.user.id;
 ...
 * 记录功能使用情况
 */
router.post('/feature-usage', auth_1.authenticate, (0, validate_1.validate)(recordFeatureUsageSchema), async (req, res) => {
    try {
        const userId = req.user.id;
 ...
 * 获取功能解锁状态
 */
router.get('/feature-unlocks', auth_1.authenticate, async (req, res) => {
    try {
        const userId = req.user.id;
 ...
 * 分析升级建议
 */
router.post('/analyze-upgrade', auth_1.authenticate, (0, validate_1.validate)(analyzeUpgradeSchema), async (req, res) => {
    try {
        const userId = req.user.id;
        ...
        if (includeHistory) {
            ...
            response.transitionHistory = transitions.map((t) => ({
                fromMode: t.fromMode,
                toMode: t.toMode,
                reason: t.reason,
 ...
 * 获取模式切换历史
 */
router.get('/transitions', auth_1.authenticate, async (req, res) => {
    try {
        const userId = req.user.id;
