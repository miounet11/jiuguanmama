...
class EmailService {
    ...
    // 生成验证码
    generateVerificationCode(length = this.CODE_LENGTH) {
    ...
    // 存储验证码
    storeVerificationCode(email, code, purpose = 'email_verification') {
    ...
    // 验证码验证
    verifyCode(email, code, purpose = 'email_verification') {
        const key = this.generateKey(email, purpose);
    ...
    // 发送账单通知
    async sendInvoiceEmail(email, username, amount, description, transactionId) {
        if (!this.transporter) {
            console.error('邮件服务不可用');
            return false;
        }
