...
class LoadBalancer extends events_1.EventEmitter {
    ...
    constructor() {
        super();
        this.config = {
            maxWorkers: Math.min(os_1.default.cpus().length, 8), // 最多8个工作进程
            minWorkers: Math.max(2, Math.floor(os_1.default.cpus().length / 2)), // 最少2个
    ...
    async stop() {
        ...
        // 优雅关闭所有工作进程
        const shutdownPromises = Array.from(this.workers.keys()).map(workerId => this.shutdownWorker(workerId));
    ...
    balanceLoad() {
        ...
        if (activeWorkers.length < this.config.maxWorkers) {
            const shouldScale = avgRequests > this.config.maxRequestsPerWorker * 0.8 ||
        ...
        if (activeWorkers.length > this.config.minWorkers) {
            const shouldDownscale = avgRequests < this.config.maxRequestsPerWorker * 0.2 &&
    ...
    getBestWorker() {
        ...
        return cluster_1.default.workers[workerId] || null;
    }
    /**
     * 获取负载均衡状态
     */
