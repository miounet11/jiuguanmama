...
exports.FeatureTrackingService = exports.FEATURE_MANIFEST = void 0;
const { PrismaClient } = require('../../node_modules/.prisma/client');
const prisma = new PrismaClient();
// 功能清单定义
exports.FEATURE_MANIFEST = [
    // 角色浏览功能
    {
...
class FeatureTrackingService {
    ...
    static async recordFeatureUsage(userId, featureId) {
        try {
            const feature = exports.FEATURE_MANIFEST.find(f => f.id === featureId);
            if (!feature) {
     ...
     * 检查功能解锁条件
     */
    static async checkFeatureUnlocks(userId, userExperience) {
        const newUnlocks = [];
        // 获取已解锁的功能
        ...
        for (const feature of exports.FEATURE_MANIFEST) {
            // 跳过已解锁的功能
            if (unlockedFeatureIds.has(feature.id))
                continue;
            // 跳过没有解锁条件的功能
            ...
            if (feature.dependencies) {
                const dependenciesMet = feature.dependencies.every(dep => unlockedFeatureIds.has(dep));
                if (!dependenciesMet)
                    continue;
            }
            // 评估解锁条件
     ...
     * 安全地评估解锁条件
     */
    static evaluateUnlockCondition(condition, userExp) {
        try {
            const context = {
            ...
            for (const op of operators) {
                if (condition.includes(op)) {
                    ...
                    if (left === 'skillLevel') {
                        ...
                        const leftIndex = skillLevels.indexOf(leftValue);
     ...
     * 更新用户技能水平
     */
    static async updateSkillLevel(userId, userExperience) {
     ...
     * 获取指定范围的功能清单
     */
    static getFeatureManifest(scope = 'global') {
        if (scope === 'global') {
            return exports.FEATURE_MANIFEST;
        }
        return exports.FEATURE_MANIFEST.filter(feature => feature.scope.includes(scope));
    }
    /**
     * 获取用户可见的功能
     */
    static async getVisibleFeatures(userId, mode, scope = 'global') {
        ...
        return manifest.filter(feature => feature.category === 'core' || unlockedIds.has(feature.id));
    }
    /**
     * 分析用户升级到专家模式的时机
     */
    static async analyzeUpgradeOpportunity(userId) {
        const userExperience = await this.getUserExperience(userId);
        ...
        const reasons = [];
        if (upgradeSignals.hasEnoughSessions)
            reasons.push('有丰富的对话经验');
        if (upgradeSignals.hasEnoughMessages)
            reasons.push('发送了大量消息');
        if (upgradeSignals.hasCreatedCharacters)
            reasons.push('创建了多个角色');
        if (upgradeSignals.hasUsedManyFeatures)
            reasons.push('使用了多种功能');
        if (upgradeSignals.hasUsedExpertFeatures)
            reasons.push('尝试了高级功能');
        if (upgradeSignals.isAdvancedUser)
            reasons.push('技能水平较高');
        return {
            shouldUpgrade,
            confidence,
...
exports.FeatureTrackingService = FeatureTrackingService;
//# sourceMappingURL=featureTrackingService.js.map