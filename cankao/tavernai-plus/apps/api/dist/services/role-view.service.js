...
class RoleViewService {
     ...
     */
    async getRoleConfig(userId, role) {
        try {
            // Get user data
            ...
            const availableFeatures = await feature_gate_service_1.featureGateService.getAvailableFeatures(userId);
     ...
     */
    async getNavigation(userId, role) {
        try {
            const navigation = [];
            // Get available features
            const availableFeatures = await feature_gate_service_1.featureGateService.getAvailableFeatures(userId);
            const unlockedFeatures = new Set(availableFeatures.filter(f => f.unlocked).map(f => f.featureId));
            // Creator navigation
            if (role === 'creator') {
                navigation.push({
                    id: 'creator-studio',
...
//# sourceMappingURL=role-view.service.js.map