...
class ChatRoomService {
    ...
    static async createRoom(data) {
        try {
            ...
            logger_1.logger.info('Chat room created', { roomId: room.id, name: data.name, ownerId: data.ownerId });
            return room;
        }
        catch (error) {
            logger_1.logger.error('Failed to create chat room', { error, data });
    ...
    static async joinRoom(data) {
        try {
            ...
            logger_1.logger.info('User joined chat room', {
                roomId: data.roomId,
                userId: data.userId,
                characterId: data.characterId
            });
            ...
            logger_1.logger.error('Failed to join chat room', { error, data });
            throw error;
        }
    }
    /**
     * 离开聊天室
     */
    static async leaveRoom(roomId, userId, characterId) {
        try {
            ...
            if (participant) {
                ...
                logger_1.logger.info('User left chat room', { roomId, userId, characterId });
            }
        }
        catch (error) {
            logger_1.logger.error('Failed to leave chat room', { error, roomId, userId, characterId });
    ...
    static async sendMessage(data) {
        try {
            ...
            logger_1.logger.info('Message sent to chat room', {
                roomId: data.roomId,
            ...
            logger_1.logger.error('Failed to send message', { error, data });
            throw error;
        }
    }
    /**
     * 召唤AI角色到聊天室
     */
    static async summonCharacter(data) {
        try {
            // 验证用户权限
            const participant = await prisma.chatParticipant.findFirst({
                where: {
            ...
            logger_1.logger.info('Character summoned to chat room', {
                roomId: data.roomId,
                characterId: data.characterId,
                userId: data.userId
            });
            ...
            logger_1.logger.error('Failed to summon character', { error, data });
            throw error;
        }
    }
    /**
     * 生成AI角色回复
     */
    static async generateCharacterResponse(roomId, characterId, userId, trigger) {
        try {
            // 获取最近的聊天历史
            ...
            // 使用增强的AI角色服务生成回复
            const aiResponse = await character_ai_1.CharacterAIService.generateCharacterResponse({
                characterId,
                userId,
                roomId,
            ...
            logger_1.logger.error('Failed to generate character response', { error, roomId, characterId });
            throw error;
        }
    }
    /**
     * 获取聊天室列表
     */
    static async getRooms(userId, page = 1, limit = 20) {
        try {
            ...
            logger_1.logger.error('Failed to get chat rooms', { error, userId });
     ...
     * 获取聊天室详情
     */
    static async getRoomDetails(roomId, userId) {
        try {
            const room = await prisma.chatRoom.findUnique({
                where: { id: roomId },
                include: {
            ...
            logger_1.logger.error('Failed to get room details', { error, roomId, userId });
            throw error;
        }
    }
    /**
     * 获取聊天消息
     */
    static async getMessages(roomId, userId, page = 1, limit = 50) {
        try {
            // 验证用户权限
            ...
            logger_1.logger.error('Failed to get messages', { error, roomId, userId });
            throw error;
        }
    }
    /**
