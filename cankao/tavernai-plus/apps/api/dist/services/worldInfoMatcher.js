...
class WorldInfoMatcher {
     ...
     */
    async findSpacetimeEntries(scenarioId, text, context = {}, depth = 3) {
        ...
        try {
            ...
            for (const entry of worldInfoEntries) {
                ...
                if (context.spacetimeAttributes && spacetimeAttributes.length > 0) {
                    const attributeMatches = context.spacetimeAttributes.filter(attr => spacetimeAttributes.includes(attr)).length;
                    spacetimeScore += attributeMatches * 0.3;
                }
                // 2. 剧情阶段权重
                if (context.currentPlotPhase && plotPhases.length > 0) {
                    const phaseConfig = plotPhases.find((p) => p.phase === context.currentPlotPhase);
                    if (phaseConfig) {
                ...
                if (culturalContext && context.spacetimeAttributes) {
                    const culturalMatches = context.spacetimeAttributes.filter(attr => culturalContext.valueSystem?.includes(attr) ||
                        culturalContext.culturalSymbols?.includes(attr)).length;
    ...
    calculateMbtiCompatibility(type1, type2) {
        ...
        // 对称填充矩阵
        const getSymmetricalCompatibility = (type1, type2) => {
