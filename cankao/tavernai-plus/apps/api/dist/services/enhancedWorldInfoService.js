...
exports.EnhancedWorldInfoService = void 0;
const { PrismaClient } = require('../../node_modules/.prisma/client');
const logger_1 = require("./logger");
class EnhancedWorldInfoService {
    prisma;
    cache;
     ...
     * 获取场景的所有世界信息条目
     */
    async getWorldInfoEntries(scenarioId, includeInactive = false) {
     ...
     * 智能匹配世界信息条目
     */
    async findMatchingEntries(scenarioId, text, context = {}, maxResults = 10) {
        logger_1.logger.info(`Finding matching entries for scenario ${scenarioId}`, {
    ...
    async createWorldInfoEntry(scenarioId, userId, data) {
        logger_1.logger.info(`Creating world info entry for scenario ${scenarioId}`, { userId });
        // 验证场景所有权
     ...
     * 更新世界信息条目
     */
    async updateWorldInfoEntry(entryId, userId, data) {
        logger_1.logger.info(`Updating world info entry ${entryId}`, { userId });
        // 验证权限
     ...
     * 删除世界信息条目
     */
    async deleteWorldInfoEntry(entryId, userId) {
        logger_1.logger.info(`Deleting world info entry ${entryId}`, { userId });
        const entry = await this.prisma.worldInfoEntry.findFirst({
            where: {
                id: entryId,
                scenario: { userId }
     ...
     * 批量重新排序条目
     */
    async reorderEntries(scenarioId, userId, entryOrders) {
        logger_1.logger.info(`Reordering entries for scenario ${scenarioId}`, { userId, count: entryOrders.length });
        // 验证场景所有权
        const scenario = await this.prisma.scenario.findFirst({
            where: { id: scenarioId, userId }
        });
        ...
        // 批量更新
        await Promise.all(entryOrders.map(({ id, displayOrder }) => this.prisma.worldInfoEntry.update({
            where: { id },
     ...
     * 智能生成相关条目
     */
    async generateRelatedEntries(scenarioId, userId, sourceEntryId, count = 3) {
        logger_1.logger.info(`Generating related entries for ${sourceEntryId}`, { count });
     ...
     * 测试关键词匹配
     */
    async testKeywordMatching(scenarioId, testText, userId) {
        logger_1.logger.info(`Testing keyword matching for scenario ${scenarioId}`, { userId });
        ...
        return entry.conditions.every(condition => {
            switch (condition.type) {
                ...
                case 'relationship_exists':
                    return context.relationships?.some(rel => rel.entityId === condition.requirement) || false;
                default:
                    return true;
            }
        });
     ...
     * 私有方法：关键词匹配
     */
    async matchKeywords(entry, text) {
            ...
            switch (entry.matchType) {
                ...
                case 'regex':
                    try {
                        ...
                        logger_1.logger.warn(`Invalid regex pattern: ${searchKeyword}`, { error });
                    }
                    break;
                case 'semantic':
     ...
     * 私有方法：计算语义相似度（简化版）
     */
    calculateSemanticSimilarity(keyword, text) {
        // 这是一个简化的语义相似度计算
     ...
     * 私有方法：生成匹配建议
     */
    generateMatchingSuggestions(matches, testText) {
        const suggestions = [];
        if (matches.length === 0) {
    ...
    transformToEnhanced(entry) {
        return {
            ...
            entryType: entry.entry_type || 'knowledge',
     ...
     * 私有方法：清除缓存
     */
    async invalidateCache(scenarioId) {
        const keys = [
...
exports.EnhancedWorldInfoService = EnhancedWorldInfoService;
exports.default = EnhancedWorldInfoService;
//# sourceMappingURL=enhancedWorldInfoService.js.map