...
exports.formatConverters = void 0;
const yaml = __importStar(require("js-yaml"));
class FormatConverters {
    /**
     * 转换为TavernAI Plus内部格式
     */
    async convertToTavernAIFormat(data, sourceFormat) {
     ...
     */
    async convertFromTavernAIFormat(scenarios, targetFormat, options = {}) {
        const startTime = performance.now();
        try {
            let data;
            const warnings = [];
     ...
     */
    async convertFromSillyTavern(data, warnings, dataLoss, fieldsIgnored) {
     ...
     * 转换SillyTavern条目
     */
    convertSillyTavernEntry(entry, warnings, fieldsIgnored) {
        try {
            // 检查必要字段
     ...
     * 转换为SillyTavern格式
     */
    async convertToSillyTavern(scenarios, options, warnings, dataLoss, fieldsIgnored) {
        const worldInfos = [];
     ...
     * 转换为SillyTavern条目
     */
    convertToSillyTavernEntry(entry, index, warnings, fieldsIgnored) {
        try {
     ...
     * 从JSON格式转换
     */
    async convertFromJSON(data, warnings, dataLoss, fieldsIgnored) {
        try {
            // 如果是字符串，先解析JSON
     ...
     * 从YAML格式转换
     */
    async convertFromYAML(data, warnings, dataLoss, fieldsIgnored) {
        try {
            let parsedData;
            // 如果是字符串，解析YAML
            if (typeof data === 'string') {
     ...
     * 从增强格式转换
     */
    async convertFromEnhanced(data, warnings, dataLoss, fieldsIgnored) {
     ...
     * 转换为JSON格式
     */
    async convertToJSON(scenarios, options, warnings, dataLoss, fieldsIgnored) {
     ...
     * 转换为YAML格式
     */
    async convertToYAML(scenarios, options, warnings, dataLoss, fieldsIgnored) {
     ...
     * 转换为增强格式
     */
    async convertToEnhanced(scenarios, options, warnings, dataLoss, fieldsIgnored) {
     ...
     * 验证和转换剧本对象
     */
    validateAndConvertScenario(item, warnings) {
...
exports.formatConverters = new FormatConverters();
//# sourceMappingURL=formatConverters.js.map