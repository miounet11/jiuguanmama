...
export class TutorialService {
   ...
   */
  async getTutorials(userId: string): Promise<TutorialConfig[]> {
    try {
      // Get user level
   ...
   * Start a tutorial
   */
  async startTutorial(userId: string, tutorialId: string): Promise<TutorialProgressData | null> {
    try {
      // Find tutorial
  ...
  async updateProgress(
    ...
    tutorialId: string,
    step: number
  ): Promise<TutorialProgressData | null> {
    try {
      const tutorial = this.tutorials.find(t => t.id === tutorialId);
      if (!tutorial) {
        throw new Error('Tutorial not found');
