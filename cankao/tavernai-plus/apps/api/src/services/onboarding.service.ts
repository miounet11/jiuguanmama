...
export class OnboardingService {
  private readonly ONBOARDING_TUTORIAL_ID = 'initial-onboarding';
  private readonly TOTAL_STEPS = 5;

  // MBTI personality mappings to character traits
  private mbtiCharacterMappings: Record<string, string[]> = {
    // Analysts
   ...
   */
  async startOnboarding(userId: string): Promise<OnboardingStatus> {
    try {
   ...
   */
  async completeStep(userId: string, stepData: OnboardingStepData): Promise<OnboardingStatus> {
    try {
    ...
    try {
      ...
      for (const char of characters) {
        ...
        const interestMatches = interests.filter(interest =>
          charTags.some((tag: string) => tag.toLowerCase().includes(interest.toLowerCase()))
        );
        ...
        if (mbtiType && preferredTraits.length > 0) {
          ...
          const traitMatches = preferredTraits.filter(
            trait => personality.includes(trait) || description.includes(trait)
          );
   ...
   * Store step data in user preferences
   */
  private async storeStepData(userId: string, stepData: OnboardingStepData): Promise<void> {
