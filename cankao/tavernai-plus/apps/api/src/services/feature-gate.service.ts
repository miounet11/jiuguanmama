...
export class FeatureGateService {
   ...
   */
  async canAccess(userId: string, featureId: string): Promise<FeatureAccessCheck> {
    try {
      // Get user data
      ...
      const requiredRoles: string[] = JSON.parse(featureConfig.requiredRoles);
      ...
      // Check dependencies
      const dependencies: string[] = JSON.parse(featureConfig.dependencies);
  ...
  async unlockFeature(
    userId: string,
    featureId: string,
    unlockMethod: 'level_up' | 'achievement' | 'payment' | 'manual' = 'manual'
  ): Promise<boolean> {
    try {
   ...
   */
  async getAvailableFeatures(userId: string): Promise<FeatureConfigData[]> {
    try {
      ...
      const unlockMap = new Map(
        unlocks.map(u => [u.featureId, { unlockedAt: u.unlockedAt, unlockMethod: u.unlockTrigger }])
      );

      // Map features with unlock status
      const result: FeatureConfigData[] = [];
   ...
   */
  async getUserUnlocks(userId: string): Promise<string[]> {
    try {
      const unlocks = await prisma.featureUnlock.findMany({
        where: { userId },
   ...
   */
  async autoUnlockFeaturesByLevel(userId: string, newLevel: number): Promise<string[]> {
    try {
