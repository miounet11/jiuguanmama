...
import { FeatureTrackingService, UserExperience } from './featureTrackingService';
...
export interface UserBehaviorPattern {
  sessionFrequency: number; // 会话频率（次/天）
  averageSessionLength: number; // 平均会话长度（分钟）
  featureExplorationRate: number; // 功能探索率
  expertFeatureAdoption: number; // 专家功能采用率
  consistencyScore: number; // 使用一致性分数
...
export class UpgradeAnalysisService {
   ...
   * 执行完整的升级分析
   */
  static async analyzeUserUpgrade(userId: string): Promise<UpgradeAnalysis> {
    try {
      // 获取用户模式信息
      ...
      // 获取用户体验数据
      const userExperience = await FeatureTrackingService.getUserExperience(userId);
   ...
   * 分析用户行为模式
   */
  private static async analyzeBehaviorPattern(userId: string): Promise<UserBehaviorPattern> {
    ...
    const totalFeatures = FeatureTrackingService.getFeatureManifest().length;
   ...
   * 计算活跃天数
   */
  private static calculateDaysWithActivity(sessions: any[], featureUsage: any[]): number {
   ...
   * 计算学习速度
   */
  private static calculateLearningVelocity(featureUsage: any[]): number {
    if (featureUsage.length === 0) return 0;

    // 按时间排序功能使用记录
    const sortedUsage = featureUsage.sort((a, b) =>
      a.firstUsedAt.getTime() - b.firstUsedAt.getTime()
    );
  ...
  private static calculateReadinessScore(
    ...
    const experienceScore = Math.min(
      (userExperience.totalSessions * 2 +
       userExperience.messagesCount * 0.1 +
       userExperience.charactersUsed * 5 +
       userExperience.featuresUsed.length * 3) / 2,
      100
    );

    // 行为分数 (0-100)
    const behaviorScore = Math.min(
      (behaviorPattern.sessionFrequency * 20 +
       behaviorPattern.featureExplorationRate * 50 +
       behaviorPattern.expertFeatureAdoption * 30),
      100
    );
  ...
  private static generateRecommendation(
    ...
    let type: 'stay' | 'upgrade' | 'consider';
   ...
   * 生成升级好处
   */
  private static generateBenefits(type: 'stay' | 'upgrade' | 'consider'): string[] {
    if (type === 'stay') {
      return [
        '界面简洁，专注核心功能',
        '学习成本低，容易上手',
   ...
   * 生成潜在风险
   */
  private static generateRisks(type: 'stay' | 'upgrade' | 'consider'): string[] {
    if (type === 'stay') {
      return [
        '可能错过强大的高级功能',
        '创作能力受到限制'
   ...
   * 为已在专家模式的用户创建分析
   */
  private static createExpertModeAnalysis(userId: string): UpgradeAnalysis {
    return {
      userId,
      currentMode: 'expert',
      recommendation: 'stay',
   ...
   * 批量分析多个用户的升级建议
   */
  static async batchAnalyzeUpgrades(userIds: string[]): Promise<UpgradeAnalysis[]> {
    const analyses = await Promise.all(
      userIds.map(userId => this.analyzeUserUpgrade(userId).catch(error => {
        console.error(`Failed to analyze user ${userId}:`, error);
        return null;
      }))
    );

    return analyses.filter(analysis => analysis !== null) as UpgradeAnalysis[];
  }

  /**
   * 获取系统级升级统计
   */
  static async getUpgradeStatistics(): Promise<{
    totalUsers: number;
