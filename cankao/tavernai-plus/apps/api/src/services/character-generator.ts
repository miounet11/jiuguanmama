...
export interface GeneratedCharacter {
  ...
  scenario: string
  firstMessage: string
  exampleDialogs: Array<{ user: string; char: string }>
  tags: string[]
  avatar?: string
  metadata: {
    generatedBy: string
...
4. Tags should accurately summarize character traits`
  ...
  private buildUserPrompt(
    ...
    if (language === 'zh-CN') {
      ...
      if (background) parts.push(`Setting: ${background}`)
    }
   ...
   * 构建图像生成提示词
   */
  private buildImagePrompt(character: GeneratedCharacter, style?: string): string {
    const parts = []
   ...
   * 使用 NAI3 生成图像
   */
  private async generateNAI3Image(prompt: string): Promise<string> {
   ...
   * 使用 DALL-E 3 生成图像
   */
  private async generateDALLE3Image(prompt: string): Promise<string> {
    const response = await axios.post(
...
4. 能引起对话兴趣`
  ...
  async enhanceCharacter(
    character: Partial<GeneratedCharacter>,
    aspects: ('personality' | 'backstory' | 'appearance' | 'all')[]
  ): Promise<Partial<GeneratedCharacter>> {
    const enhanced = { ...character }

    for (const aspect of aspects) {
      if (aspect === 'all' || aspect === 'personality') {
        enhanced.personality = await this.enhanceAspect(
      ...
      if (aspect === 'all' || aspect === 'backstory') {
        enhanced.backstory = await this.enhanceAspect(
          character.backstory || '',
          'backstory',
          character.name || ''
        )
      ...
      if (aspect === 'all' || aspect === 'appearance') {
        enhanced.appearance = await this.enhanceAspect(
