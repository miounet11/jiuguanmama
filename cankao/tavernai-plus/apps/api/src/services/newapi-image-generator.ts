 ...
 * 支持角色头像、对话背景图生成
 */
export class NewAPIImageGenerator {
   ...
   */
  async generateBackground(character: Character): Promise<string> {
   ...
   * 根据MBTI类型获取视觉风格
   */
  private getMBTIVisualStyle(mbtiType?: string | null): string {
    if (!mbtiType) return '';
   ...
   * 根据MBTI类型获取背景氛围
   */
  private getMBTIAmbiance(mbtiType: string): string {
    const mbtiAmbiances: Record<string, string> = {
      // 分析家(NT)
   ...
   * 从性格描述中提取视觉关键词
   */
  private extractVisualKeywords(personality: string): string {
   ...
   * 从背景故事中提取环境关键词
   */
  private extractEnvironmentKeywords(backstory: string): string {
