 ...
 */
export const validateExtensionAccess = async (
  ...
  try {
    ...
    const userExtensions = await extensionService.getInstalledExtensions(userId);
    ...
    logger.error('Extension access validation failed', {
 ...
 */
export const checkExtensionPermissions = (requiredPermissions: string[]) => {
  return (req: ExtensionRequest, res: Response, next: NextFunction) => {
    try {
      if (!req.extension) {
        return res.status(400).json({
      ...
      logger.error('Extension permission check failed', {
 ...
 */
export const requireExtensionEnabled = (
  req: ExtensionRequest,
  res: Response,
  next: NextFunction
) => {
  try {
    ...
    logger.error('Extension enabled check failed', {
      extensionId: req.extension?.id,
      error: error instanceof Error ? error.message : String(error)
    });
...
export const extensionRateLimit = (
  maxExecutionsPerMinute: number = 10,
  maxSandboxContexts: number = 3
) => {
  const userExecutionCounts = new Map<string, { count: number; resetTime: number }>();

  return async (req: ExtensionRequest, res: Response, next: NextFunction) => {
    try {
      const userId = req.user?.id;
      ...
      const userContexts = activeContexts.filter(ctx =>
      ...
      logger.error('Extension rate limit check failed', {
        userId: req.user?.id,
        extensionId: req.extension?.id,
        error: error instanceof Error ? error.message : String(error)
      });
...
export const securityAudit = (
  ...
  try {
    ...
    if (isSecurityEvent) {
      logger.warn('Extension security event', {
        event: `${method} ${url}`,
        userId,
        extensionId,
      ...
      if (method === 'POST' && url.includes('/execute')) {
        ...
        if (dangerousFunctions.some(fn =>
          functionName?.includes(fn) || JSON.stringify(args)?.includes(fn)
        )) {
          logger.error('Potentially dangerous extension execution attempt', {
            userId,
            extensionId,
            functionName,
...
export const executionMonitor = (
  ...
  res.json = function(data: any) {
    ...
    logger.info('Extension execution completed', {
      extensionId: req.extension?.id,
      userId: req.user?.id,
      method: req.method,
      url: req.url,
    ...
    if (req.extension?.id) {
      ...
      // For now, just log it
      logger.debug('Extension activity updated', {
        extensionId: req.extension.id,
        timestamp: new Date().toISOString()
      });
    }
 ...
 */
export const sandboxManager = async (
  req: ExtensionRequest,
  res: Response,
  next: NextFunction
) => {
  try {
    ...
    // Log sandbox status
    logger.debug('Sandbox status check', {
      extensionId,
    ...
    if (sandboxStats.activeContexts > 50) {
      logger.warn('High sandbox load detected', {
        activeContexts: sandboxStats.activeContexts,
        memoryUsage: sandboxStats.memoryUsage
      });
    }
    ...
    if (sandboxStats.activeContexts > 100) {
      logger.error('Emergency sandbox cleanup triggered', {
        activeContexts: sandboxStats.activeContexts
      });

      // Kill old contexts for this extension
      const extensionContexts = activeContexts.filter(ctx =>
        ctx.extensionId === extensionId &&
        Date.now() - ctx.lastActivity.getTime() > 300000 // 5 minutes
      );

      for (const context of extensionContexts) {
        await sandboxService.destroyContext(context.id);
      }
    }

    next();
  } catch (error) {
    logger.error('Sandbox management failed', {
      extensionId: req.extension?.id,
      error: error instanceof Error ? error.message : String(error)
    });
 ...
 */
export const validateExtensionUpload = (
  req: Request,
  res: Response,
  next: NextFunction
) => {
  try {
    ...
    // Log upload attempt
    logger.info('Extension file upload attempt', {
      userId: req.user?.id,
    ...
    logger.error('Extension upload validation failed', {
      userId: req.user?.id,
      error: error instanceof Error ? error.message : String(error)
    });
 ...
 */
export const extensionSecurityMiddleware = [
  securityAudit,
  executionMonitor,
  sandboxManager
];
 ...
 */
export const extensionExecutionMiddleware = [
