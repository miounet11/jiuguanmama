...
// 图像生成相关路由
// 图像提示模板
router.get('/image/prompt-templates', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    // 返回预设的图像生成提示模板
    const templates = [
      {
...
router.get('/image/recent-generations', authenticate, async (req: AuthRequest, res: Response) => {
  try {
...
router.get('/image/settings', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    // 返回用户图像生成设置
    const settings = {
      model: 'dall-e-3',
...
router.get('/voice/settings', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    // 返回用户语音设置
    const settings = {
