...
router.get('/recommended', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (characters.length < limit) {
      const popular = await prisma.character.findMany({
        where: {
          isPublic: true,
          NOT: {
...
// 收藏角色
router.post('/characters/:id/favorite', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    const userId = req.user!.id
...
// 取消收藏角色
router.delete('/characters/:id/favorite', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    const userId = req.user!.id
    const characterId = req.params.id
...
// 评价角色
router.post('/characters/:id/rate', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    const userId = req.user!.id
    const characterId = req.params.id
