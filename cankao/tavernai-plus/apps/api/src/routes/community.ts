...
// 用户关注系统
// 关注用户
router.post('/users/:id/follow', authenticate, async (req: AuthRequest, res: Response) => {
  try {
...
// 取消关注
router.delete('/users/:id/follow', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    const userId = req.user!.id
    const targetUserId = req.params.id
...
// 编辑评论
router.put('/comments/:id', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    const userId = req.user!.id
...
// 取消点赞动态
router.delete('/posts/:id/like', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    const userId = req.user!.id
    const postId = req.params.id
...
// 分享动态
router.post('/posts/:id/share', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    const userId = req.user!.id
    const postId = req.params.id
...
// 点赞评论
router.post('/comments/:id/like', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    const userId = req.user!.id
    const commentId = req.params.id
...
// 获取用户创建的角色
router.get('/users/:id/characters', async (req: Request, res: Response) => {
  try {
    const userId = req.params.id
    const page = parseInt(req.query.page as string) || 1
    const limit = parseInt(req.query.limit as string) || 20
