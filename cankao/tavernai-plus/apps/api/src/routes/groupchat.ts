...
interface GroupChatSettings {
  ...
  speakingMode: 'free' | 'ordered' | 'moderated' | 'natural'
  autoModeration: boolean
  allowUserMessages: boolean
  messageDelay: number
  conversationStyle: 'natural' | 'structured' | 'roleplay'
  worldInfoIds: string[]
  contextSettings: {
    maxTokens: number
...
router.post('/', authenticate, async (req: AuthRequest, res, next) => {
  try {
    ...
    // 默认设置
    const defaultSettings: GroupChatSettings = {
      maxParticipants: 10,
      speakingMode: 'natural',
      autoModeration: true,
      allowUserMessages: true,
...
// 处理群组自动回复逻辑
async function handleGroupAutoReplies(groupId: string, triggerMessage: any) {
  try {
    // TODO: 从数据库获取群组信息和参与者
    // TODO: 根据设置决定哪些角色应该回复
...
// 添加角色到群组
router.post('/:id/participants', authenticate, async (req: AuthRequest, res, next) => {
  try {
    const { id } = req.params
...
// 移除群组参与者
router.delete('/:id/participants/:participantId', authenticate, async (req: AuthRequest, res, next) => {
  try {
...
// 更新群组设置
router.put('/:id/settings', authenticate, async (req: AuthRequest, res, next) => {
  try {
    const { id } = req.params
