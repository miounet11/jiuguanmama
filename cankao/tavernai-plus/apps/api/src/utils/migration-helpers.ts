  ...
  async migrateCharacterJsonFields() {
    ...
    for (const character of characters) {
      ...
      try {
        ...
        const newTags = stringifyCharacterTags(tags)
        ...
        updates.tags = stringifyCharacterTags([])
        needsUpdate = true
      }
      ...
      if (character.exampleDialogs) {
        try {
          const dialogs = parseCharacterExampleDialogs(character.exampleDialogs)
          const newDialogs = stringifyCharacterExampleDialogs(dialogs)
          ...
          updates.exampleDialogs = stringifyCharacterExampleDialogs([])
          needsUpdate = true
        }
      }
      ...
      if (character.metadata === null || character.metadata === undefined) {
        updates.metadata = stringifyCharacterMetadata(DEFAULT_CHARACTER_METADATA)
        needsUpdate = true
      } else {
        try {
          const metadata = parseCharacterMetadata(character.metadata)
          const newMetadata = stringifyCharacterMetadata(metadata)
  ...
  async migrateChatSessionJsonFields() {
    ...
    for (const session of sessions) {
      ...
      if (session.metadata === null || session.metadata === undefined) {
        updates.metadata = stringifyChatSessionMetadata({})
        needsUpdate = true
      } else {
        try {
          const metadata = parseChatSessionMetadata(session.metadata)
          const newMetadata = stringifyChatSessionMetadata(metadata)
