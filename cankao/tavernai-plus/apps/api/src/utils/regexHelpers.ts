...
import { RegexCacheItem, LRUCache } from '../types/worldInfo'

// 正则表达式编译缓存
class RegexCache implements LRUCache<string, RegexCacheItem> {
  private cache = new Map<string, RegexCacheItem>()
  ...
  get(key: string): RegexCacheItem | undefined {
  ...
  set(key: string, value: RegexCacheItem): void {
...
export function safeCompileRegex(
  ...
  try {
    ...
    // 缓存编译结果
    const cacheItem: RegexCacheItem = {
      pattern,
      regex,
      flags,
      compiled: new Date()
 ...
 * 计算括号嵌套深度
 */
function countNestedParentheses(pattern: string): number {
  let maxDepth = 0
  let currentDepth = 0
 ...
 * 创建全词匹配的正则表达式
 */
export function createWholeWordRegex(word: string, flags: string = 'gi'): RegExp | null {
 ...
 * 创建包含匹配的正则表达式
 */
export function createContainsRegex(text: string, flags: string = 'gi'): RegExp | null {
 ...
 * 创建开始匹配的正则表达式
 */
export function createStartsWithRegex(text: string, flags: string = 'gi'): RegExp | null {
  const escaped = escapeRegexChars(text)
 ...
 * 创建结束匹配的正则表达式
 */
export function createEndsWithRegex(text: string, flags: string = 'gi'): RegExp | null {
  const escaped = escapeRegexChars(text)
  const pattern = `${escaped}$`
  return safeCompileRegex(pattern, flags)
}
 ...
 * 批量编译正则表达式
 */
export function compileRegexBatch(
  patterns: Array<{ pattern: string; flags?: string }>
): Array<{ pattern: string; regex: RegExp | null; error?: string }> {
...
): { averageTime: number; matchCount: number } {
 ...
 * 支持正则表达式、通配符、逻辑操作符
 */
export function parseSillyTavernKeywords(keywordString: string): {
  exact: string[]
  regex: RegExp[]
  wildcard: RegExp[]
  ...
  const result = {
    ...
    regex: [] as RegExp[],
    wildcard: [] as RegExp[],
    operator: 'AND_ANY' as 'AND_ANY' | 'AND_ALL' | 'NOT_ANY' | 'NOT_ALL',
    caseSensitive: false,
    wholeWord: false
  }
 ...
 * 验证关键词字符串格式
 */
export function validateKeywordSyntax(keywordString: string): {
  isValid: boolean
  errors: string[]
  warnings: string[]
} {
